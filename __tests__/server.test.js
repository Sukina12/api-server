  ...
  describe ('Status and returned data', () => {
    ...
    describe ('/clothes', () =>{
      ...
      it ('PUT  record', async () => {
        ...
        let response = await mockRequest.put(`${route}/${id}`).send(clothes);
        expect(response.body.name).toEqual('T-Shirt');
        expect(response.body.quantity).toEqual('5');
        expect(response.status).toEqual(200);
      });
      it ('GET record by ID', async () => {
        let route = '/api/v1/clothes';
        let response = await mockRequest.get(`${route}/${id}`);
    ...
    describe ('/food', () =>{
      ...
      it ('PUT  record', async () => {
        ...
        let response = await mockRequest.put(`${route}/${id}`).send(food);
        expect(response.body.name).toEqual('Burger');
        expect(response.body.quantity).toEqual('5');
        expect(response.status).toEqual(200);
      });
